...
async def ping(ctx):
    ...
    embed = create_embed(ctx,title="Ping",fields=[{'name': "Your ping",'value': ping_content}])
    await ctx.channel.send(embed=embed)

@commands.command(brief="Get bot information.")
async def about(ctx):
    about_content = "Created by **Gary Hilares**.\nLicensed under **MIT License**.\nContact: **dev.garynlol@gmail.com**."
    embed = create_embed(ctx,title="About",fields=[{'name': "Bot information", 'value': about_content}])
    await ctx.channel.send(embed=embed)
...
async def role(ctx, *args):
    ...
    if role_name == '':
        ...
        embed = create_embed(ctx,title="Roles",fields=[{'name': "Autoassignable roles", 'value': roles_content}])
        await ctx.channel.send(embed=embed)
    else:
        role = get(member.guild.roles,name=role_name)
        if role and role_name in assignable_roles:
            await member.add_roles(role)
            await ctx.send(f"Assigned {role_name} role to {ctx.author}.")
        else:
            await ctx.send(f"Couldn't assign {role_name} role to {ctx.author}.")
...
async def help(ctx):
    embed = create_embed(ctx,title="Help",fields=[])
